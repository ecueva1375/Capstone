...
AUTH0_DOMAIN = 'ecfsnd.us.auth0.com'
ALGORITHMS = ['RS256']
...
# Auth Header
# get_token_auth_header() method attempt
# to get the header from the request.
# Raise an AuthError if no header is present.
# Split bearer and the token and raise an AuthError
...
def get_token_auth_header():
    ...
    if parts_token[0].lower() != 'bearer':
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Authorization header must start with "Bearer".'
        }, 401)

    elif len(parts_token) == 1:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Token not found.'
        }, 401)

    elif len(parts_token) > 2:
        raise AuthError({
            'code': 'invalid_header',
...
# Verify that it is an Auth0 token with key id (kid).
